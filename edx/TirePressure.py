...
#Add your code here!
def tire_pressure(int_measurements):
    got5 = 0
    counter = 0
    valid_list = []
    while got5 < 5:
        index = -(counter + 1)
        if int_measurements[index] <= 55 and int_measurements[index] >= 15:
            valid_list.append(int_measurements[index])
            got5 += 1
        counter += 1

    sum = 0
    for reading in valid_list:
        sum += reading
