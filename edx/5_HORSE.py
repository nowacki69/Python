...
def check_horse_winner(tpl_Status):
    count = 0
    players = ""
    for i in range(len(tpl_Status)):
        if len(tpl_Status[i]) < 5:
            count += 1
            if count == 1:
                players += str(i + 1)
            else:
    ...
    if count == len(tpl_Status):
        msgText = "Everyone: keep playing!"
    elif count == 1:
        msgText = "Player " + players + " wins!"
    else:
        msgText = "Players " + players + ": keep playing!"
...
print(check_horse_winner(("HORS", "HOR", "HORSE", "HORSE")))
