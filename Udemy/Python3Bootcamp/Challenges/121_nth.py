# Write a function called nth, which accepts a list and a number and returns the element at whatever index is the
# number in the list. If the number is negative, the nth element from the end is returndself.

# You can assume that number will always be between the negative value of the list length and the list length minus 1.

def nth(aList, number):
    return aList[number]
